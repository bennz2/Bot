...
module.exports = {
  config: {
    name: "captcha",
    description: "This  Commands Used For captcha",
    usage: "captcha",
    aliases: [],
  },
  run: async (bot, message, args) => {
      ...
      (attachmentsss
        ...
        ? args[0].length == 18
          ...
          : message.guild.members.cache
              .find((r) => r.user.username.toLowerCase() === args.join(" ").toLocaleLowerCase())
    ...
    const res = await fetch(encodeURI(`https://nekobot.xyz/api/imagegen?type=captcha&username=${name}&url=${imagetarget}`));
