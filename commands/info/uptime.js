...
module.exports = {
  config: {
    name: "uptime",
    aliases: ["upbot"],
    description: "Info Of Uptime Bots",
    usage: "uptime",
  },
  run: async (bot, message, args) => {
    ...
    const milliseconds = parseInt((bot.uptime % 1000) / 100);
    let seconds = parseInt((bot.uptime / 1000) % 60);
    let minutes = parseInt((bot.uptime / (1000 * 60)) % 60);
    let hours = parseInt((bot.uptime / (1000 * 60 * 60)) % 24);
    let days = parseInt((bot.uptime / (1000 * 60 * 60 * 24)) % 60);
    ...
    const derweSA = bot.mapss.get("uptimedate");
    const serveddrembed = new Discord.MessageEmbed().setDescription(`${emojis.loading} Fetching Uptime...`).setColor("RED");
