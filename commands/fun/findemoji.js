...
module.exports = {
  ...
  run: async (bot, message, args) => {
    ...
    let noResult = new discord.MessageEmbed().setDescription(`| :x: No Results found for ${args.join(" ")}!`).setColor("FF2052");
    ...
    while (doing) {
      let reaction;
      try {
        reaction = await msg.awaitReactions(filter, { max: 1, time: 120000, errors: ["time"] });
      } catch {
        message.channel.send(message.author.toString() + ", You took too long.");
